...
class KinovaArm:
    ACTION_TIMEOUT_DURATION = 40

    def __init__(self):
        # Check if arm is connected
        try:
            subprocess.run(['ping', '-c', '1',  '10.0.3.26'], check=True, timeout=1, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    def move_cartesian_vel(self, cartesian_vel, blocking=True):
        ...
        twist.linear_x = cartesian_vel[0]
        twist.linear_y = cartesian_vel[1]
        twist.linear_z = cartesian_vel[2]

        twist.angular_x = cartesian_vel[3]
        twist.angular_y = cartesian_vel[4]
        twist.angular_z = -cartesian_vel[5]
...
if __name__ == '__main__':
    ...
    try:
        ...
        arm.home()
        for i in range(10):
            arm.move_cartesian_vel([0.1, 0.1, 0.1])
        #angles = [318.8551,  39.2031,   1.3355,  67.4939, 334.6591,  64.2477,  25.0062]
        #angles = [262.184326171875, 128.0, 148.15472412109375, 147.0, 180.0, 102.07113647460938, 71.56495666503906]# arm.close_gripper()
        # arm.toss()
        # angles[1] = 10
        #arm.movej_primitive.execute(angles, max_vel=5, max_accel=10, max_decel=10, gripper_release_ms=600)
        # angles[1] = 20
        # arm.movej_primitive.execute(angles, max_vel=10, max_accel=30, max_decel=20, gripper_release_ms=600)
        # angles[1] = 30
        # arm.movej_primitive.execute(angles, max_vel=10, max_accel=30, max_decel=20, gripper_release_ms=600)
